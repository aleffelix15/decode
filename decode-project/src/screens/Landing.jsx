...
export function Landing({ onStart, onHow }) {
  return (
    <div className="min-h-screen flex flex-col items-center justify-center px-4 sm:px-6 py-12 sm:py-16 text-center relative overflow-hidden">
      ...
      <div className="relative z-10 max-w-lg w-full">
        ...
        <p className="mt-6 sm:mt-8 text-xs sm:text-sm leading-relaxed px-1" style={{ color: C.sub }}>Você vai analisar situações, tomar decisões e descobrir sinais de violência que muitas vezes são confundidos com cuidado.</p>
        <div className="mt-8 sm:mt-10 flex flex-col gap-2 sm:gap-3 justify-center">
          <PrimaryButton onClick={onStart} className="flex items-center gap-2 justify-center text-xs sm:text-sm">
            <Play size={14} className="sm:block hidden" /> INICIAR EXPERIÊNCIA
          </PrimaryButton>
          <GhostButton onClick={onHow} className="text-xs sm:text-sm">COMO FUNCIONA</GhostButton>
        </div>
      </div>
    </div>
  );
