...
export function StoryIntro({ onStart, onExit, label = "A TERCEIRA PESSOA", subhead, role = "rede de apoio" }) {
  return (
    <div className="max-w-lg mx-auto text-center py-6 sm:py-8 md:py-10 px-1">
      ...
      <GlitchTitle size="text-2xl xs:text-3xl sm:text-4xl md:text-5xl">{label}</GlitchTitle>
      <p className="mt-3 sm:mt-4 text-sm leading-relaxed" style={{ color: C.sub }}>
        {subhead || '"Você não está vivendo essa história.\nVocê está aprendendo a perceber os sinais."'}
      </p>
      <div className="mt-5 sm:mt-6 flex flex-col gap-1.5 sm:gap-2 text-sm leading-relaxed" style={{ color: C.text }}>
        <p>Você será a {role}.</p>
