      ...
      <ChatShell transition={null} messages={CHAPTER5.messages} visibleMsgs={visible} typing={typing}>
        {allShown && !confirmed && (
          <Panel className="p-4 max-w-xl">
            <p className="font-bold text-sm mb-1 tracking-wide" style={{ color: C.text }}>{CHAPTER5.question}</p>
            ...
            <div className="flex flex-col gap-2 mb-4">
              {CHAPTER5.checklist.map((item, i) => (
                <label key={i} className="flex items-center gap-3 px-4 py-3 rounded-lg border text-sm font-medium cursor-pointer" style={{ borderColor: checked[i] ? C.lilac : C.line, background: C.panel2, color: C.text }}>
                  <input type="checkbox" checked={!!checked[i]} onChange={() => toggle(i)} className="w-4 h-4 focus:outline-none focus-visible:ring-2" style={{ accentColor: C.lilac }} />
                  {item.text}
                </label>
              ))}
            </div>
