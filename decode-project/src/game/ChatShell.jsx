...
export function ChatShell({ transition, messages, visibleMsgs, typing, children }) {
  return (
    <div>
      {transition && (
        <p className="text-center text-[11px] font-semibold tracking-widest mb-4" style={{ color: C.sub }}>{transition}</p>
      )}
      <Panel className="p-4 mb-4 max-w-xl">
        <div className="flex items-center gap-2 mb-4 pb-3 border-b" style={{ borderColor: C.line }}>
          <div className="w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold" style={{ background: rgba(C.lilac, 0.25), color: C.text }} aria-hidden="true">S</div>
          <div>
        ...
        <div className="flex flex-col gap-2 min-h-[100px]" aria-live="polite" aria-relevant="additions">
          {messages.slice(0, visibleMsgs).map((m, i) => (
            <div key={i} className="self-start max-w-[85%] px-3 py-2 rounded-xl rounded-tl-sm text-sm" style={{ background: C.panel2, color: C.text }}>{m}</div>
          ))}
          {typing && (
